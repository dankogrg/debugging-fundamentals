    ...
    try {
        if (sharedVariables.nameUser) {
            ...
            document.querySelector(
                "#greeting"
            ).innerHTML = `Hello ${sharedVariables.nameUser}`;
            btnStart.focus();
        }
        ...
        if (btnProfile) {
            btnProfile.addEventListener("click", () => {
                ...
                const usernameProfile =
                    document.querySelector("#usernameProfile");
    ...
    try {
        if (btnConfig) {
            btnConfig.addEventListener("click", () => {
                ...
                document.querySelector("#usernameConfig").innerHTML =
                    sharedVariables.nameUser;
...
export function handleUsername() {
    try {
        sharedVariables.inputName.placeholder =
            checkFields.generatePlaceholder();
    } catch (error) {
        console.error(
            `Error inserting random placeholder if input username is empty: ${error}`
        );
    }
}
