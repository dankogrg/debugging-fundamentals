class Ranking {
    ...
    createRanking(data) {
        ...
        if (!data || data.length === 0) {
            ...
            noResultsCell.textContent =
                "No ranking available for the selected period";
            ...
            if (data.length < this.limit) {
                document.getElementById("showMoreResults").style.display =
                    "none";
            } else {
                document.getElementById("showMoreResults").style.display =
                    "block";
    ...
    async handlePeriodChange(periodRanking, limit) {
        ...
        try {
            ...
            if (!data || data.length === 0) {
                document.getElementById("showRankingDiv").style.display =
                    "none";
                document.getElementById("showMoreResults").style.display =
                    "none";
            } else {
                document.getElementById("showRankingDiv").style.display =
                    "block";
                document.getElementById("btnsPeriodRanking").style.display =
                    "block";
                document.getElementById("showMoreResults").style.display =
                    "block";
